...
function postImageToFacebook( authToken, filename, mimeType, imageData, message )
{
    ...
    xhr.open( 'POST', 'https://graph.facebook.com/v2.2/me/photos?access_token=' + authToken, true );
...
function postCanvasToFacebook(sourceCanvas, message) {
	var data = sourceCanvas.toDataURL("image/png");
	var encodedPng = data.substring(data.indexOf(',') + 1, data.length);
	var decodedPng = Base64Binary.decode(encodedPng);
	//var message = "image upload test http://rectangleworld.com";
	...
	FB.login(function(response) {// do the following once login happens
		...
		if (response.authResponse) {// check what happened
			...
			alert("Posted to Facebook! (Please go to your Facebook page to verify, in case of any glitches.)");
		}
		else {
			 alert("user login problem!");
